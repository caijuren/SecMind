...
> 本计划与 spec 文档共用一套术语，先约定再使用，避免后文歧义。

### 架构与代码层

| 术语 | 定义 | 出处 |
|---|---|---|
| **AtomicCapability**（原子能力） | 单一职责的调查动作，输入 → 数据源查询 → 输出 `CapabilityResult`。不调用其他 capability（依赖必须显式声明）。 | `core/capability.py` |
| **Namespace**（命名空间） | 能力的全局唯一标识，形如 `investigate.<category>.<name>`，LLM Agent 通过它发现并路由调用。 | spec §0 / `core/registry.py` |
| **CapabilityResult** | 所有能力的统一输出信封，含 `payload / confidence / partial / evidence_refs / pii_fields / duration_ms`。 | `core/schema.py` |
| **EvidenceRef** | 指向原始记录的指针（source, query, record_id, captured_at），用于审计与"反查原始数据"。 | `core/schema.py` |
| **PIIMarker** | 标记 payload 中某个 JSONPath 字段含 PII（name/email/phone/...），交给下游 redactor 按调用者角色裁剪。 | `core/schema.py` |
| **ExecutionContext** | 单次能力执行的运行时上下文，持有 `datasources` 句柄、`trace_id`、`caller_role`。 | `core/context.py` |
| **CapabilityRegistry** | namespace → AtomicCapability 类的注册表，支持装饰器注册和前缀枚举。 | `core/registry.py` |
| **DataSource (DS-XXX)** | 抽象数据源接口；同一接口可挂多家具体厂商实现（Splunk、CrowdStrike、Okta 等）。本仓库只提供 ABC + Mock + 一个参考实现。 | `datasources/base.py` |

### 错误与降级语义

| 术语 | 定义 | 行为 |
|---|---|---|
| **DataSourceUnavailable** | 数据源不可达（超时、限速、鉴权失败）。 | 在 `run()` 中转换为 `partial=True, confidence=0.0`，**不抛出**。 |
| **PartialResult** | 能力主动声明"我有部分数据，但不完整"。 | 携带 `payload + reasons + confidence`，在 `run()` 中转换为 `partial=True`。 |
| **CapabilityFatal** | 不可恢复的错误（不变量被打破）。 | **抛出**，由上层编排器决定中断或重试。 |
| **partial_reasons** | 列出每个降级原因，格式 `"DS-XXX: <reason>"`，便于 LLM 决策是否补查。 | `CapabilityResult.partial_reasons` |

### 数据源缩写

| 缩写 | 含义 | 典型厂商 |
|---|---|---|
| **DS-IDP** | Identity Provider，账户/登录/MFA/OAuth | AD, Okta, Azure AD, Ping |
| **DS-HR** | HR 系统，员工/部门/在职状态 | Workday, BambooHR, 飞书人事 |
| **DS-CMDB** | 资产管理库，主机/资产/业务上下文 | ServiceNow CMDB, 自建 |
| **DS-EDR** | 终端检测响应，进程/网络/文件/Hash | CrowdStrike, SentinelOne, Defender |
| **DS-SIEM** | 安全日志聚合，跨源关联检索 | Splunk, Elastic, Sentinel |
| **DS-EMAIL** | 邮件系统，邮件/收发/转发规则 | Exchange Online, Gmail Workspace |
| **DS-FW** | 防火墙/流量，连接/会话/策略命中 | Palo Alto, Fortinet |
| **DS-TI** | 威胁情报，IP/域名/URL/Hash 信誉 | VirusTotal, MISP, AlienVault OTX |
| **DS-KB** | 知识库，历史告警/已知 IOC/相似案例 | 自建 ES 索引 |
| **DS-NDR**（P1） | 网络检测响应，会话/JA3 | Vectra, ExtraHop |
| **DS-CLOUD**（P1） | 云审计，IAM/资源/API | CloudTrail, Azure Activity, GCP Audit |
| **DS-SANDBOX**（P1） | 文件/URL 沙箱 | CAPE, JoeSandbox, ANY.RUN |
| **DS-PAM**（P1） | 特权访问，会话录像/票据 | CyberArk, BeyondTrust |
| **DS-VULN**（P1） | 漏洞扫描 + KEV/EPSS | Tenable, Qualys |
| **DS-WHOIS**（P1） | 域名注册 + 被动 DNS + 证书 | DomainTools, SecurityTrails |

**圆点标注约定**：`●` 表示必需数据源（缺失即 partial），`○` 表示可选数据源（缺失不影响主结论）。

### 编排术语

| 术语 | 定义 |
|---|---|
| **编排（Orchestration）** | LLM Agent 按业务目标串联多个原子能力的过程；本仓库**只提供积木**，编排逻辑由 Agent / 上层规则引擎实现。 |
| **触发场景（Trigger）** | 哪一类告警 / 哪个上游能力的输出会驱动本能力被调用。 |
| **上游（Upstream）** | 在编排链中提供本能力输入字段的前序能力或 raw alert 字段。 |
| **下游（Downstream）** | 消费本能力输出的后续能力。 |
| **金标用例（Golden case）** | 用于验收的固定输入 → 期望输出对，写在测试中作为契约。 |
| **TTP** | Tactics, Techniques, Procedures（MITRE ATT&CK 三层抽象）。 |
| **IOC** | Indicator of Compromise（IP/域名/URL/Hash/邮箱/Mutex/证书指纹）。 |
| **Kill-chain** | 攻击链阶段（Recon → Weaponization → Delivery → Exploit → Install → C2 → Action）。 |

### 判定动词

| 动词 | 含义 |
|---|---|
| **TP** | True Positive，确认为真实攻击。 |
| **FP** | False Positive，误报。 |
| **BP** | Benign Positive，行为属实但属预期业务/维护活动。 |
| **INC** | Inconclusive，证据不足无法判定。 |
...
**示例编排（Example Orchestration）:**

- **触发场景**：任一告警的 `subject_user` 字段非空时（钓鱼、暴破、数据外传、离职人员异常等），调查的第一步几乎都会先调本能力建立人的全貌。
- **上游**：raw alert 字段 `principal` / `subject_user` / `actor.email`；或 B1(登录) 异常事件的 `principal`。
- **下游**：
  - D2(用户身份上下文) 复用 `identity.status / leaving_date` 做进一步分类
  - F2(影响评估) 用 `privilege.is_privileged` 与 `risk_tags` 提升严重性
  - F8(处置建议) 据 `auth.mfa_enabled / locked` 决定建议 reset_password 还是 disable_account

```python
# 钓鱼场景：告警 victim=alice@corp
user = await UserProfileCapability().run(
    {"principal": alert["subject_user"], "time_window_days": 30},
    ctx,
)
if "external_forwarding" in user.payload["risk_tags"]:
    # 立刻分叉到邮件深查 + 处置建议
    email_act = await EmailActivityCapability().run(
        {"mailbox": alert["subject_user"], "time_window_days": 7}, ctx
    )
    # 喂给 TP/FP 与处置
    verdict = await TpFpCapability().run(
        {"evidence_set": [user.model_dump(), email_act.model_dump()]}, ctx
    )
```
...
- **示例编排**：
  - **触发**：告警含 `host` / `endpoint_id` / `src_ip`；勒索/挖矿/横向移动调查必查。
  - **上游**：raw alert `host_id`；A4(IP 画像)解析内网 IP → host 后回查本能力。
  - **下游**：B3(进程)、B4(网络)、C3(进程链) 以 `host` 为锚点；F2(影响)读 `criticality`；F8(处置)对 `criticality=critical` 走"先隔离+保留证据"路径。
  ```python
  host = await HostProfileCapability().run({"host": alert["host_id"]}, ctx)
  if host.payload["asset"]["criticality"] == "critical":
      # 关键资产 → 不能直接 isolate，先深查
      procs = await ProcessExecutionCapability().run(
          {"host": alert["host_id"], "time_window_minutes": 30}, ctx
      )
  ```
...
- **示例编排**：
  - **触发**：B1(登录) 中 `src_ip`、B4(网络) 中外联 IP、防火墙告警的 `dst_ip`。
  - **上游**：B1 / B4 / raw alert 的 IP 字段。
  - **下游**：内网 IP → A2(主机画像)；外网 IP → E1(IP 信誉) + C9(victim expansion P1)；C2(实体图)以 IP 为边。
  ```python
  ip_prof = await IpProfileCapability().run({"ip": alert["src_ip"]}, ctx)
  if ip_prof.payload["classification"] == "internal" and ip_prof.payload["internal_resolution"]:
      host = await HostProfileCapability().run(
          {"host": ip_prof.payload["internal_resolution"]["host"]}, ctx
      )
  elif ip_prof.payload["reputation"]["verdict"] == "malicious":
      ioc = await IocExtractCapability().run({"evidence_set": [ip_prof.model_dump()]}, ctx)
  ```
...
- **示例编排**：
  - **触发**：B8(邮件) 中链接的域名、B5(DNS P1) 查询的域名、proxy/EDR 中的 URL host。
  - **上游**：B8 / B5 / raw alert `url` 字段（先 parse 出 domain）。
  - **下游**：E2(域名信誉)用作 verdict 部分；F4(IOC 提取)；F1(TP/FP)。
  ```python
  dom = await DomainProfileCapability().run(
      {"domain": "c0rp-login.com"}, ctx
  )
  if "brand_lookalike" in dom.payload.get("lookalike", {}).get("technique", []):
      # 触发组合判定：新域名 + 像 c0rp → 强 phishing 信号
      verdict_evidence.append(dom.model_dump())
  ```
...
- **示例编排**：
  - **触发**：B3(进程) 中的 `image_path/hash`、B8(邮件) 中的附件、EDR 告警的 file event。
  - **上游**：B3 / B8(附件) / raw alert `file_hash`。
  - **下游**：E3(Hash 信誉)；C9(victim expansion P1)沿 `internal_spread.hosts` 横扫。
  ```python
  file_prof = await FileProfileCapability().run(
      {"hash": proc["hash"]}, ctx
  )
  if file_prof.payload["reputation"]["verdict"] == "malicious":
      # 已感染主机列表交给横向扩散调查
      affected = file_prof.payload["internal_spread"]["hosts"]
  ```
...
- **示例编排**：
  - **触发**：账户告警（暴破/异地登录/MFA bypass）、内部用户视角调查、横向移动前置。
  - **上游**：A1(用户画像) 提供 principal；或 raw alert 直接给 principal。
  - **下游**：A4(IP 画像) 解每个 `src_ip`；D5(基线) 比对历史；F1(TP/FP)。
  ```python
  logins = await LoginActivityCapability().run(
      {"principal": user.payload["auth"]["upn"], "time_window_hours": 24}, ctx
  )
  for anomaly in logins.payload["anomalies"]:
      if anomaly["type"] == "impossible_travel":
          ip_a = await IpProfileCapability().run({"ip": anomaly["ip_a"]}, ctx)
          ip_b = await IpProfileCapability().run({"ip": anomaly["ip_b"]}, ctx)
  ```
...
- **示例编排**：
  - **触发**：EDR 告警、勒索/挖矿/横向移动调查、A2 主机画像产出异常进程清单。
  - **上游**：A2(主机画像)；或 raw alert 的 `process_guid`。
  - **下游**：C3(进程链)；E7(TTP→MITRE)；A7(file 画像)对 image_path/hash；F4(IOC 提取)。
  ```python
  procs = await ProcessExecutionCapability().run(
      {"host": host_id, "time_window_minutes": 30}, ctx
  )
  for p in procs.payload["tree"]:
      if p["suspicion_score"] >= 70:
          lineage = await ProcessLineageCapability().run({"process_guid": p["guid"]}, ctx)
          ttps = await TtpMitreCapability().run(
              {"evidence_set": [{"kind": "cmdline", "content": p["cmdline"]}]}, ctx
          )
  ```
...
- **示例编排**：
  - **触发**：C2 告警、外联可疑域名、B3 中可疑进程附带 PID 后查其网络行为。
  - **上游**：B3(进程 GUID)、A2(主机)；防火墙告警。
  - **下游**：A4(IP)、A5(域名)；E1(IP 信誉)；C9(victim expansion P1)。
  ```python
  conns = await NetworkConnectionCapability().run(
      {"process_guid": proc["guid"], "time_window_minutes": 60}, ctx
  )
  if any(a["type"] == "beacon_detected" for a in conns.payload["anomalies"]):
      for dst in conns.payload["top_destinations"][:5]:
          await IpProfileCapability().run({"ip": dst["ip"]}, ctx)
  ```
...
- **示例编排**：
  - **触发**：钓鱼告警、A1 检测到 `external_forwarding`、邮件网关上报。
  - **上游**：A1(用户) 提供 mailbox；raw alert `message_id`。
  - **下游**：A7(file 画像) 处理附件；A5(域名) / E2(URL 信誉) 处理链接；F4(IOC)。
  ```python
  emails = await EmailActivityCapability().run({"message_id": alert["message_id"]}, ctx)
  for msg in emails.payload["messages"]:
      for link in msg.get("links", []):
          await DomainUrlReputationCapability().run({"target": link, "kind": "url"}, ctx)
      for att in msg.get("attachments", []):
          await FileProfileCapability().run({"hash": att["hash"]}, ctx)
  ```
...
- **示例编排**：
  - **触发**：任何"以告警 ts ± window 找其他线索"的步骤；C5/F6 重建时间线的底层原语。
  - **上游**：raw alert 的 `(ts, entity)`；F1 不确定时常调用本能力补证据。
  - **下游**：F6(时间线重建 P1)；F1(TP/FP) 用作旁证。
  ```python
  related = await CooccurrenceCapability().run(
      {
          "anchor": {"ts": alert["ts"], "entity_kind": "host", "entity_id": alert["host_id"]},
          "window_seconds": 600,
          "pivot_keys": ["host", "user", "ip"],
      },
      ctx,
  )
  ```
...
- **示例编排**：
  - **触发**：调查中后期需要"画一张图"评估爆炸半径；F2(影响)前置；攻击链可视化。
  - **上游**：A1/A2/A4 已确认主要实体后。
  - **下游**：F2(影响) 用 `nodes.count by kind`；C5(横向 P1) 路径回溯。
  ```python
  graph = await EntityGraphCapability().run(
      {"seed": {"kind": "user", "id": "alice@corp"}, "depth": 2,
       "edge_types": ["owner", "logon", "conn"]},
      ctx,
  )
  blast_users = [n for n in graph.payload["nodes"] if n["kind"] == "user"]
  ```
...
- **示例编排**：
  - **触发**：B3 检出高 suspicion 进程后，追溯调用链。
  - **上游**：B3(进程) 的 `process_guid`。
  - **下游**：E7(TTP→MITRE) 在 lineage 上识别 LOLBin 模式；F9(根因 P1) 推断初始入口。
  ```python
  lineage = await ProcessLineageCapability().run({"process_guid": proc["guid"]}, ctx)
  # 根进程往往是初始入口（document opening / service / scheduled task）
  root = lineage.payload["lineage_tree"]["root"]
  ```
...
- **示例编排**：
  - **触发**：A2(主机画像) 之后，凡是要评估"动它损失多大"的决策点。
  - **上游**：A2(host) 的 `asset.id`。
  - **下游**：F2(影响) 输入；F8(处置) 决定是否 `requires_approval=true`、是否避开维护窗。
  ```python
  biz = await AssetBusinessCapability().run({"asset_id": host.payload["asset"]["id"]}, ctx)
  if biz.payload["in_maintenance"]:
      verdict_hints.append("BP_candidate")  # 维护窗内行为更可能是 BP
  ```
...
- **示例编排**：
  - **触发**：调查中需"对人物分类"的所有节点（VIP / 离职 / 特权 / 普通）。
  - **上游**：A1(用户) 已得到 principal。
  - **下游**：F1(TP/FP) 在判定里加权；F2(影响) 决定 severity；F8(处置) 决定走"温和"还是"快速隔离"路径。
  ```python
  uctx = await UserIdentityContextCapability().run({"principal": "alice@corp"}, ctx)
  if uctx.payload["watchlist"]:
      severity_bias = "+1"  # watchlist 用户的告警提升 1 档
  ```
...
- **示例编排**：
  - **触发**：B1/B3/B4 检测到 anomaly 后，确认是否真的"反常"。
  - **上游**：B1/B3/B4 的具体事件。
  - **下游**：F1(TP/FP) 用 deviation 加权；G1/G3(假设/下一步 P1)。
  ```python
  baseline = await SelfBaselineCapability().run(
      {"entity": {"kind": "user", "id": "alice@corp"},
       "dimensions": ["hour_of_day", "geo", "device"], "lookback_days": 30},
      ctx,
  )
  if baseline.payload["geo"]["deviation"] > 3.0:
      anomaly_strength["geo_new"] = True
  ```
...
- **示例编排**：
  - **触发**：A4(IP 画像) 已确定 IP 为外部；B4(网络) 中的外联 IP。
  - **上游**：A4 / B4。
  - **下游**：F1(TP/FP)、F4(IOC 提取)。
  ```python
  rep = await IpReputationCapability().run({"ip": "1.2.3.4"}, ctx)
  if rep.payload["verdict"] in {"malicious", "suspicious"}:
      verdict_evidence.append(rep.model_dump())
  ```
...
- **示例编排**：
  - **触发**：B8(邮件)中链接、B5(DNS P1)查询、proxy 日志中的 URL。
  - **上游**：A5(域名画像) 或 raw alert 的 URL。
  - **下游**：F1(TP/FP)、F4(IOC)。
  ```python
  for link in email_msg["links"]:
      r = await DomainUrlReputationCapability().run({"target": link, "kind": "url"}, ctx)
      if r.payload["verdict"] == "malicious":
          ioc_set.add(link)
  ```
...
- **示例编排**：
  - **触发**：B3(进程) 中的进程 hash、B8(邮件) 附件 hash、A7(file 画像) 输出。
  - **上游**：A7 或 raw alert 的 `file_hash`。
  - **下游**：F1(TP/FP)、F4(IOC)、E5(沙箱 P1)（如 family 未知再去 detonate）。
  ```python
  hr = await HashReputationCapability().run({"hash": proc["hash"]}, ctx)
  if hr.payload["av_detection_ratio"] > 0.3:
      verdict_evidence.append(hr.model_dump())
  ```
...
- **示例编排**：
  - **触发**：拿到 B3 进程列表或 C3 进程链后，识别"这是什么手法"。
  - **上游**：B3 / C3 / B8 中的行为线索。
  - **下游**：C4(kill-chain P1)；F1(TP/FP)；F8(处置) 按 tactic 选不同响应剧本。
  ```python
  ttps = await TtpMitreCapability().run(
      {"evidence_set": [{"kind": "cmdline", "content": p["cmdline"]} for p in procs.payload["tree"]]},
      ctx,
  )
  tactics = {t["tactic"] for t in ttps.payload["ttps"]}
  ```
...
- **示例编排**：
  - **触发**：每次调查链的"收口"环节；所有 evidence-gathering capability 跑完后调用一次。
  - **上游**：所有 A/B/C/D/E 类 capability 的 `CapabilityResult`。
  - **下游**：F2(影响)、F8(处置)、G4(回查) 都依赖本结果。
  ```python
  verdict = await TpFpCapability().run(
      {"evidence_set": [u.model_dump(), e.model_dump(), r.model_dump(), p.model_dump()]},
      ctx,
  )
  if verdict.payload["verdict"] == "INC":
      next_step = await NextStepCapability().run(
          {"current_state": {"evidence_set": [...], "hypotheses": []}}, ctx
      )
  ```
...
- **示例编排**：
  - **触发**：F1 出 TP/BP 后，需要给 SOC/管理层一个 severity。
  - **上游**：F1(verdict)、D1(资产)、D2(用户)、C2(实体图)。
  - **下游**：F8(处置) 按 severity 决定是否 `requires_approval` / 通知 oncall。
  ```python
  impact = await ImpactCapability().run(
      {"verdict_payload": verdict.payload, "asset_ctx": biz.payload,
       "user_ctx": uctx.payload, "affected_entities": graph.payload["nodes"]},
      ctx,
  )
  if impact.payload["severity"] in {"P0", "P1"}:
      page_oncall()
  ```
...
- **示例编排**：
  - **触发**：F1 判定为 TP 后，输出可分发的 IOC（推送 EDR/FW/TI 黑名单）。
  - **上游**：所有产生外部 artifact 的 capability（A4/A5/A7/B3/B4/B8）。
  - **下游**：F8(处置) 的 `block_ip`/`block_domain` 直接消费；外部 TI 平台。
  ```python
  iocs = await IocExtractCapability().run(
      {"evidence_set": [u.model_dump(), email_act.model_dump(), url_rep.model_dump()]},
      ctx,
  )
  push_to_blocklist(iocs.payload["iocs"])
  ```
...
- **示例编排**：
  - **触发**：F1 出 TP/BP + F2 出 severity 后；调查链的最后一步。
  - **上游**：F1, F2, F4(IOC)。
  - **下游**：SOAR / 工单系统消费 actions；高 severity 项触发审批工作流。
  ```python
  resp = await ResponseRecommendationCapability().run(
      {"verdict": verdict.payload, "impact": impact.payload,
       "affected_entities": {"users": [...], "hosts": [...]}},
      ctx,
  )
  for action in resp.payload["containment"]:
      if action["requires_approval"]:
          send_for_approval(action)
      else:
          soar.execute(action)
  ```
...
- **示例编排**：
  - **触发**：F1 输出 `verdict=INC`、或调查进行到一半 LLM Agent 想问"下一步去查什么"。
  - **上游**：当前 evidence_set。
  - **下游**：Agent 据 `recommended_next_atomic_calls` 决定下一次 `Registry.lookup(ns).run(...)`。
  ```python
  while verdict.payload["verdict"] == "INC" and iter < MAX:
      nxt = await NextStepCapability().run(
          {"current_state": {"evidence_set": evidence_set, "hypotheses": []}}, ctx
      )
      best = nxt.payload["recommended_next_atomic_calls"][0]
      cap_cls = default_registry.lookup(best["namespace"])
      new_evidence = await cap_cls().run(best["expected_input"], ctx)
      evidence_set.append(new_evidence.model_dump())
      verdict = await TpFpCapability().run({"evidence_set": evidence_set}, ctx)
  ```
...
- **示例编排**：
  - **触发**：调查链开始时（找先验）或 F1 出 verdict 后（用历史回看校准）。
  - **上游**：F4(IOC)、E7(TTP)、raw alert 类型 → 拼成 fingerprint。
  - **下游**：F1(TP/FP) 加权（"过去 5 次同类全是 FP"是强先验）；F8(处置) 借鉴 past_response。
  ```python
  similar = await KbLookupCapability().run(
      {"fingerprint": {
          "alert_type": alert["type"],
          "primary_iocs": [i["value"] for i in iocs.payload["iocs"]],
          "ttps": [t["id"] for t in ttps.payload["ttps"]],
      }},
      ctx,
  )
  if similar.payload and similar.payload[0]["similarity"] > 0.8:
      prior_verdict_bias = similar.payload[0]["past_verdict"]
  ```
