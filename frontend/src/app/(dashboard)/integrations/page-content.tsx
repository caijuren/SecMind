...
export function IntegrationsPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      setApps(appsResponse.data.items?.length > 0 ? appsResponse.data.items : MOCK_APPS)
      setWebhooks(webhookResponse.data.items?.length > 0 ? webhookResponse.data.items : MOCK_WEBHOOKS)
    } catch {
      setApps(MOCK_APPS)
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        icon={Plug}
        title={t("integrations.pageTitle")}
        subtitle={<span>{t("integrations.pageSubtitle")}</span>}
      />

      <div className="grid gap-4 md:grid-cols-3">
        {[
          { key: "integrated" as const, labelKey: "integrations.tabIntegrated" as const, count: apps.filter((item) => item.source === "integrated").length, icon: Link2 },
          { key: "marketplace" as const, labelKey: "integrations.tabMarketplace" as const, count: apps.filter((item) => item.source === "marketplace").length, icon: Plus },
          { key: "webhooks" as const, labelKey: "integrations.tabWebhooks" as const, count: webhooks.length, icon: Webhook },
        ].map((item) => (
          <button key={item.key} onClick={() => setTab(item.key)} role="tab" aria-selected={tab === item.key} className={`${softCardClass} p-4 text-left ${tab === item.key ? "ring-2 ring-cyan-500/30" : ""}`}>
            <div className="flex items-center justify-between">
              ...
              <span className="rounded-lg bg-primary/10 p-2 text-cyan-600"><item.icon className="size-4" /></span>
            </div>
          </button>
        ))}
      </div>
      ...
      <div className={`${softCardClass} flex flex-col gap-3 p-4 md:flex-row md:items-center`}>
        <div className="relative flex-1">
          ...
          <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t("integrations.searchPlaceholder")} className={`pl-9 ${inputClass}`} aria-label={t("integrations.searchAriaLabel")} name="search" type="search" autoComplete="off" />
        </div>
        {tab === "webhooks" && (
      ...
      {tab !== "webhooks" && (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <Card key={app.id} className={pageCardClass}>
              <CardContent className="space-y-4 p-5">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Badge variant="outline" className={app.status === "connected" ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-600" : "border-border bg-muted/50 text-muted-foreground"}>
                    {app.status === "connected" ? <CheckCircle2 className="mr-1 size-3.5" /> : <XCircle className="mr-1 size-3.5" />}
                    {app.status === "connected" ? t("integrations.statusConnected") : t("integrations.statusDisconnected")}
                  </Badge>
                </div>
                <div className="text-xs text-muted-foreground">{t("integrations.lastSync")}{app.last_sync || "-"}</div>
                <div className="flex gap-2">
                  {app.source === "integrated" ? (
                    <Button variant="outline" size="sm" onClick={() => setSelectedApp(app)}>
                      <Settings2 className="mr-1 size-3.5" />
                      {t("integrations.btnConfig")}
                    </Button>
                  ) : (
                    <Button size="sm" onClick={() => setSelectedMarketApp(app)}>
          ...
          {!loading && filteredApps.length === 0 && <div className="text-muted-foreground">{t("integrations.noMatch")}</div>}
        </div>
      )}
              ...
              <CardContent className="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between">
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge variant="outline" className={webhook.active ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-600" : "border-border bg-muted/50 text-muted-foreground"}>
                      {webhook.active ? t("integrations.webhookActive") : t("integrations.webhookInactive")}
                    </Badge>
                  </div>
                  <p className="font-mono text-sm text-muted-foreground">{webhook.url}</p>
                  <div className="flex flex-wrap gap-2">
                    {webhook.events.map((event) => (
                      <Badge key={event} variant="outline" className="border-cyan-500/20 bg-primary/10 text-cyan-600">{event}</Badge>
                    ))}
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Button variant="outline" size="sm" onClick={() => toggleWebhook(webhook)}>
                    {webhook.active ? t("integrations.btnDisable") : t("integrations.btnEnable")}
                  </Button>
                  <Button variant="outline" size="sm">
                    <Send className="mr-1 size-3.5" />
                    {t("integrations.btnTest")}
                  </Button>
                </div>
              </CardContent>
            </Card>
      ...
      <Dialog open={!!selectedApp} onOpenChange={() => setSelectedApp(null)}>
        <DialogContent className="border-border bg-card text-foreground">
          <DialogHeader>
            <DialogTitle>{t("integrations.configTitle")} {selectedApp?.name}</DialogTitle>
            <DialogDescription className="text-muted-foreground">{t("integrations.configDescription")}</DialogDescription>
          </DialogHeader>
          {selectedApp && (
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="app-api-url">{t("integrations.labelApiUrl")}</Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="app-api-key">{t("integrations.labelApiKey")}</Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="app-sync-freq">{t("integrations.labelSyncFrequency")}</Label>
                <Select value={selectedApp.sync_frequency} onValueChange={(value) => setSelectedApp({ ...selectedApp, sync_frequency: value || '' })}>
                  ...
                  <SelectContent>
                    <SelectItem value="5min">{t("integrations.freq5min")}</SelectItem>
                    <SelectItem value="15min">{t("integrations.freq15min")}</SelectItem>
                    <SelectItem value="30min">{t("integrations.freq30min")}</SelectItem>
                    <SelectItem value="1h">{t("integrations.freq1h")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={() => setSelectedApp(null)}>{t("integrations.btnCancel")}</Button>
                <Button onClick={() => saveAppConfig({ ...selectedApp, status: "connected" })}>{t("integrations.btnSaveConfig")}</Button>
              </div>
            </div>
          )}
        </DialogContent>
      ...
      <Dialog open={!!selectedMarketApp} onOpenChange={() => setSelectedMarketApp(null)}>
        <DialogContent className="border-border bg-card text-foreground">
          <DialogHeader>
            <DialogTitle>{t("integrations.connectTitle")} {selectedMarketApp?.name}</DialogTitle>
            <DialogDescription className="text-muted-foreground">{t("integrations.connectDescription")}</DialogDescription>
          </DialogHeader>
          {selectedMarketApp && (
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="market-api-url">{t("integrations.labelApiUrl")}</Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="market-auth-key">{t("integrations.labelAuthKey")}</Label>
              ...
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={() => setSelectedMarketApp(null)}>{t("integrations.btnCancel")}</Button>
                <Button onClick={addMarketplaceApp}>{t("integrations.btnConfirmConnect")}</Button>
              </div>
            </div>
          )}
        </DialogContent>
      ...
      <Dialog open={webhookDialogOpen} onOpenChange={setWebhookDialogOpen}>
        <DialogContent className="border-border bg-card text-foreground">
          <DialogHeader>
            <DialogTitle>{t("integrations.webhookDialogTitle")}</DialogTitle>
            <DialogDescription className="text-muted-foreground">{t("integrations.webhookDialogDescription")}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="webhook-name">{t("integrations.labelName")}</Label>
            ...
            <div className="space-y-2">
              <Label htmlFor="webhook-events">{t("integrations.labelEvents")}</Label>
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setWebhookDialogOpen(false)}>{t("integrations.btnCancel")}</Button>
              <Button onClick={createWebhook}>{t("integrations.btnCreate")}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
