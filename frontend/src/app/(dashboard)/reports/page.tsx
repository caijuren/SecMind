...
function GenerateReportDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg border-white/[0.06] bg-[#131316] text-zinc-100 shadow-lg">
        <DialogHeader>
          <DialogTitle className="text-zinc-100">生成报告</DialogTitle>
        ...
        <div className="space-y-5 py-2">
          <div className="space-y-2">
            <label className="text-xs font-medium text-zinc-600">报告类型</label>
            <Select value={reportType} onValueChange={(v) => v && setReportType(v)}>
                <SelectTrigger className="h-9 border-white/[0.06] bg-[#131316] text-sm text-zinc-300 focus:ring-cyan-400/20">
                  <SelectValue placeholder="选择报告类型" />
                </SelectTrigger>
                <SelectContent className="border-white/[0.06] bg-[#131316]">
                  <SelectItem value="daily" className="text-zinc-400 focus:bg-cyan-500/10 focus:text-cyan-400">日报</SelectItem>
                  <SelectItem value="weekly" className="text-zinc-400 focus:bg-cyan-500/10 focus:text-cyan-400">周报</SelectItem>
                  <SelectItem value="monthly" className="text-zinc-400 focus:bg-cyan-500/10 focus:text-cyan-400">月报</SelectItem>
                  <SelectItem value="special" className="text-zinc-400 focus:bg-cyan-500/10 focus:text-cyan-400">专项报告</SelectItem>
                </SelectContent>
              </Select>
          </div>
          <div className="space-y-2">
            <label className="text-xs font-medium text-zinc-600">时间范围</label>
            <Select value={timeRange} onValueChange={(v) => v && setTimeRange(v)}>
                <SelectTrigger className="h-9 border-white/[0.06] bg-[#131316] text-sm text-zinc-300 focus:ring-cyan-400/20">
                  <SelectValue placeholder="选择时间范围" />
                </SelectTrigger>
                <SelectContent className="border-white/[0.06] bg-[#131316]">
                  <SelectItem value="today" className="text-zinc-400 focus:bg-cyan-500/10 focus:text-cyan-400">今日</SelectItem>
                  <SelectItem value="yesterday" className="text-zinc-400 focus:bg-cyan-500/10 focus:text-cyan-400">昨日</SelectItem>
                  <SelectItem value="this_week" className="text-zinc-400 focus:bg-cyan-500/10 focus:text-cyan-400">本周</SelectItem>
                  <SelectItem value="last_week" className="text-zinc-400 focus:bg-cyan-500/10 focus:text-cyan-400">上周</SelectItem>
                  <SelectItem value="this_month" className="text-zinc-400 focus:bg-cyan-500/10 focus:text-cyan-400">本月</SelectItem>
                  <SelectItem value="last_month" className="text-zinc-400 focus:bg-cyan-500/10 focus:text-cyan-400">上月</SelectItem>
                </SelectContent>
              </Select>
          </div>
          <div className="space-y-2">
            <label className="text-xs font-medium text-zinc-600">包含模块</label>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected
                        ...
                        : "border-white/[0.06] bg-[#131316] text-zinc-600 hover:border-white/[0.08] hover:text-zinc-400"
                    )}
                  >
                    {isSelected ? (
          ...
          <div className="flex justify-end gap-2 pt-2">
            <Button
              ...
              className="border-white/[0.06] text-zinc-600 hover:bg-white/[0.04] hover:text-zinc-300"
            >
              取消
            </Button>
            <Button
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-zinc-100">报告中心</h1>
            <p className="text-sm text-zinc-600 mt-0.5">安全运营报告自动生成与管理</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-zinc-700" />
          <Input
            placeholder="搜索报告..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="h-9 border-white/[0.06] bg-[#131316] pl-9 text-sm text-zinc-300 placeholder:text-zinc-700 focus-visible:border-cyan-400/60 focus-visible:ring-cyan-400/20"
            aria-label="搜索报告"
            name="search"
            type="search"
            autoComplete="off"
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold text-zinc-200 flex items-center gap-2">
          ...
          报告列表
          <span className="text-xs text-zinc-700 font-normal">({filteredReports.length})</span>
        </h2>
        {filteredReports.map((report) => {
          ...
          return (
            <Card
              key={report.id}
              className="border-white/[0.06] bg-[#131316] hover:border-cyan-500/20 transition-colors"
            >
              <CardContent className="p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0 space-y-3">
                    <div className="flex items-center gap-2.5">
                      <span className="text-xs font-mono tabular-nums text-zinc-600">{report.id}</span>
                      <Badge
                        variant="outline"
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <Button
                      variant="ghost"
                      size="icon-sm"
                      className="text-zinc-700 hover:text-cyan-400 hover:bg-cyan-500/10"
        ...
        <div className="grid grid-cols-3 gap-4">
          {mockTemplates.map((tpl) => {
            ...
            return (
              <Card
                key={tpl.id}
                className={`${softCardClass} hover:border-cyan-500/20 transition-colors cursor-pointer group`}
              >
                <CardContent className="p-5 space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm font-semibold text-zinc-100 group-hover:text-cyan-400 transition-colors">{tpl.name}</h3>
                    </div>
                  </div>
                  <p className="text-xs text-zinc-500 leading-relaxed">{tpl.description}</p>
                  <div className="flex items-center justify-between pt-1">
                    <span className="text-[10px] text-zinc-500">已使用 {tpl.usageCount} 次</span>
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-6 text-[10px] text-zinc-400 hover:text-cyan-400 hover:bg-cyan-500/10 gap-1"
                    >
