...
const CATEGORY_CONFIG: Record<string, { labelKey: string; count: number; color: string; icon: typeof User }> = {
  user_action: { labelKey: "audit.categoryUserAction", count: 1247, color: "#06b6d4", icon: User },
  system_change: { labelKey: "audit.categorySystemChange", count: 389, color: "#a855f7", icon: Settings },
  data_access: { labelKey: "audit.categoryDataAccess", count: 2156, color: "#22c55e", icon: Database },
}

const OPERATION_TYPE_KEY: Record<OperationType, string> = {
  "登录": "audit.typeLogin",
  "查询": "audit.typeQuery",
  "修改": "audit.typeModify",
  "删除": "audit.typeDelete",
...
export function AuditPage() {
  ...
  const categoryCards = Object.entries(CATEGORY_CONFIG) as [string, { labelKey: string; count: number; color: string; icon: typeof User }][]
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-3 gap-4">
        {categoryCards.map(([key, config]) => {
          ...
          return (
            <Card
              key={key}
              className={cn(
                "cursor-pointer transition-colors overflow-hidden border",
                isActive && "ring-1"
              )}
              style={{
                background: isActive
                  ...
                  : `${config.color}06`,
                borderColor: isActive ? `${config.color}40` : `${config.color}20`,
                boxShadow: isActive ? "0 1px 3px rgba(0,0,0,0.3)" : "0 1px 2px rgba(0,0,0,0.2)",
              }}
              onClick={() => { setActiveCategory(isActive ? "all" : key as FilterCategory); setCurrentPage(1) }}
            >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      >
                        ...
                        <span className="text-xs font-normal text-muted-foreground/60 ml-1">{t("common.items")}</span>
                      </p>
                    </div>
                  </div>
                  <div
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Select value={typeFilter} onValueChange={(v) => { setTypeFilter(v ?? "all"); setCurrentPage(1) }}>
            ...
            <SelectContent className="border-cyan-500/25 bg-card text-foreground">
          ...
          <Select value={timeFilter} onValueChange={(v) => { setTimeFilter(v ?? "7d"); setCurrentPage(1) }}>
      ...
      <div className="rounded-lg border border-border bg-card overflow-hidden">
          <table className="w-full caption-bottom text-sm">
            <thead>
              <tr className="border-b border-border hover:bg-transparent">
                <th className="h-10 px-4 text-left align-middle font-medium whitespace-nowrap font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{t("audit.colTime")}</th>
                <th className="h-10 px-4 text-left align-middle font-medium whitespace-nowrap font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{t("audit.colOperator")}</th>
                <th className="h-10 px-4 text-left align-middle font-medium whitespace-nowrap font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{t("audit.colOperationType")}</th>
                <th className="h-10 px-4 text-left align-middle font-medium whitespace-nowrap font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{t("audit.colTarget")}</th>
                <th className="h-10 px-4 text-left align-middle font-medium whitespace-nowrap font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{t("audit.colDetail")}</th>
                <th className="h-10 px-4 text-left align-middle font-medium whitespace-nowrap font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{t("audit.colIP")}</th>
                <th className="h-10 px-4 text-left align-middle font-medium whitespace-nowrap font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{t("audit.colResult")}</th>
              </tr>
            </thead>
            <tbody>
              {paginatedLogs.map((log) => {
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-2.5 align-middle whitespace-nowrap text-xs font-mono text-muted-foreground">{log.time}</td>
                    <td className="px-4 py-2.5 align-middle whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={cn("text-xs font-medium", isSystem ? "text-purple-600" : "text-foreground")}>
                          {log.operator}
                        </span>
                      </div>
                    </td>
                    <td className="px-4 py-2.5 align-middle whitespace-nowrap">
                      ...
                      >
                        {t(OPERATION_TYPE_KEY[log.operationType])}
                      </Badge>
                    </td>
                    <td className="px-4 py-2.5 align-middle whitespace-nowrap text-xs text-muted-foreground font-medium">{log.target}</td>
                    <td className="px-4 py-2.5 align-middle whitespace-nowrap text-xs text-muted-foreground max-w-[280px] truncate">{log.detail}</td>
                    <td className="px-4 py-2.5 align-middle whitespace-nowrap text-xs font-mono text-muted-foreground">{log.ip}</td>
                    <td className="px-4 py-2.5 align-middle whitespace-nowrap">
                      {log.result === "成功" ? (
                        <Badge
                          variant="outline"
          ...
          <TablePagination
            ...
            resultsLabel={t("audit.resultsCount")}
            perPageLabel={t("audit.paginationPerPage")}
          />
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            className="border-cyan-500/25 bg-primary/10 text-cyan-600 hover:bg-cyan-500/20 hover:border-primary/30 gap-2"
          >
          ...
          <Button
            variant="outline"
            className="border-border bg-muted/50 text-muted-foreground hover:bg-muted/50 hover:border-border gap-2"
          >
            <Download className="h-3.5 w-3.5" />
            {t("audit.exportPDF")}
          </Button>
