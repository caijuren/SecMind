...
const resourceKeyMap: Record<string, string> = {
  alert: "rbac.res.alert",
  response: "rbac.res.response",
  hunting: "rbac.res.hunting",
  investigate: "rbac.res.investigate",
  dashboard: "rbac.res.dashboard",
  report: "rbac.res.report",
  playbook: "rbac.res.playbook",
  user: "rbac.res.user",
  system: "rbac.res.system",
  integration: "rbac.res.integration",
  contact: "rbac.res.contact",
  "*": "rbac.res.superPermission",
}

const actionKeyMap: Record<string, string> = {
  read: "rbac.actionLabel.read",
  write: "rbac.actionLabel.write",
  execute: "rbac.actionLabel.execute",
  approve: "rbac.actionLabel.approve",
  cancel: "rbac.actionLabel.cancel",
  "*": "rbac.actionLabel.all",
}

const rbacColorMap: Record<string, string> = {
  cyan: "bg-primary/10 text-cyan-600",
...
function CreateRoleDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="border-border bg-card text-foreground sm:max-w-lg max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t("rbac.createRoleDialogTitle")}</DialogTitle>
          <DialogDescription className="text-muted-foreground">
            {t("rbac.createRoleDialogDesc")}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="role-name">{t("rbac.roleIdentifier")}</Label>
            <Input
              id="role-name"
              name="name"
              autoComplete="off"
              ...
              placeholder={t("rbac.roleIdentifierPlaceholder")}
          ...
          <div className="space-y-2">
            <Label>{t("rbac.selectPermissions")}</Label>
              ...
              {Array.from(grouped.entries()).map(([resource, perms]) => (
                <div key={resource}>
                  <p className={`text-xs font-medium text-muted-foreground mb-1.5`}>
                    {t(resourceKeyMap[resource] || resource)}
                  </p>
                      ...
                      <button
                        ...
                        className={`inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-xs transition-colors ${
                          selectedPermIds.includes(p.id)
                            ? "border-cyan-500/30 bg-primary/10 text-cyan-600"
            ...
            <p className={`text-xs ${subtleTextClass}`}>
              {selectedPermIds.length + t("rbac.selectedPermissionsCount")}
            </p>
          </div>
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              disabled={saving}
            >
              {saving ? t("rbac.creating") : t("rbac.createRole")}
            </Button>
          </div>
        </div>
      </DialogContent>
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="border-border bg-card text-foreground sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-muted-foreground">
            {t("rbac.assignRoleDialogDesc")}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="assign-user-id">{t("rbac.userId")}</Label>
            <Input
              id="assign-user-id"
              name="user_id"
              autoComplete="off"
              ...
              placeholder={t("rbac.userId")}
              type="number"
              className={inputClass}
            />
          </div>
          <div className="space-y-2">
            <Label>{t("rbac.selectRoles")}</Label>
              ...
              {roles.map((role) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {role.is_system === 1 && (
                      <Badge variant="outline" className="border-amber-500/25 bg-amber-500/10 text-amber-600 text-[10px] px-1">
                        {t("rbac.system")}
                      </Badge>
                    )}
                  </div>
          ...
          {result && (
            <div className="rounded-lg border border-emerald-500/25 bg-emerald-500/10 p-3 space-y-1">
              <div className="flex items-center gap-1.5 text-sm font-medium text-emerald-600">
                ...
                {t("rbac.assignSuccess")}
              </div>
              <p className={`text-xs text-emerald-600`}>
                {t("rbac.rolesLabel") + (result.roles.join(", ") || t("rbac.noRoles"))}
              </p>
              <p className={`text-xs text-emerald-600`}>
                {t("rbac.permissionsCountLabel") + result.permissions.length}
              </p>
            </div>
          )}
          <div className="flex justify-end gap-2">
            ...
            >
              {saving ? t("rbac.assigning") : t("rbac.confirmAssign")}
            </Button>
          </div>
        </div>
      </DialogContent>
    ...
    <Card className={CARD.elevated}>
      <CardContent className="p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className={String(TYPOGRAPHY.h2)}>{t("rbac.userPermissionQuery")}</h2>
            <p className={String(TYPOGRAPHY.caption) + " text-muted-foreground mt-0.5"}>
              {t("rbac.userPermissionQueryDesc")}
            </p>
          </div>
        </div>
        <div className="flex gap-2 mb-4">
          ...
          <Button
            ...
            className="shrink-0"
          >
            {loading ? t("rbac.querying") : t("rbac.query")}
          </Button>
        </div>
        {error && (
          <div className="rounded-lg border border-red-500/25 bg-red-500/10 p-3 text-sm text-red-600">
            {error}
          </div>
        )}
        {result && (
            ...
            <div className="rounded-lg border border-border bg-muted/50 p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className={`text-sm font-medium text-muted-foreground`}>
                  {t("rbac.userRoles").replace("{{id}}", String(result.user_id))}
                </span>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {result.roles.length > 0 ? (
                  result.roles.map((role) => (
                    <Badge
                      key={role}
                      variant="outline"
                      className="border-cyan-500/25 bg-primary/10 text-cyan-600"
                    >
                      {role}
                    </Badge>
                  ))
                ) : (
                  <span className={`text-xs ${subtleTextClass}`}>{t("rbac.noRole")}</span>
                )}
              </div>
            </div>
            <div className="rounded-lg border border-border bg-muted/50 p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className={`text-sm font-medium text-muted-foreground`}>
                  {t("rbac.permissionList").replace("{{count}}", String(result.permissions.length))}
                </span>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {result.permissions.length > 0 ? (
                  result.permissions.map((perm) => (
                    <Badge
                      key={perm}
                      variant="outline"
                      className="border-border bg-muted/50 text-muted-foreground text-[11px]"
                    >
                      {perm}
                    </Badge>
                  ))
                ) : (
                  <span className={`text-xs ${subtleTextClass}`}>{t("rbac.noPermission")}</span>
                )}
              </div>
            </div>
          </div>
...
export function RbacPage() {
  ...
  async function handleSeed() {
    ...
    try {
      ...
      setSeedMessage(res.data?.message || t("rbac.initComplete"))
      await loadAll()
    } catch {
      setSeedMessage(t("rbac.initFailed"))
    } finally {
      setSeeding(false)
  ...
  return (
    <div className="space-y-5">
      ...
      {seedMessage && (
        <div className="flex items-center gap-2 rounded-lg border border-emerald-500/25 bg-emerald-500/10 px-4 py-2.5 text-sm text-emerald-600">
          <CheckCircle2 className="size-4" />
          {seedMessage}
        </div>
      )}
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {[
          { label: t("rbac.totalRoles"), value: stats.total, icon: Shield, color: "cyan" },
          ...
          <div key={item.label} className={`${softCardClass} p-4`}>
            <div className="flex items-center justify-between">
              ...
              <span
                className={`rounded-lg ${rbacColorMap[item.color] ?? "bg-muted/50 text-muted-foreground"} p-2`}
              >
      ...
      <Tabs defaultValue="roles">
        ...
        <TabsContent value="roles" className="mt-4 space-y-4">
          ...
          {loading ? (
            <div className="p-10 text-center text-muted-foreground">{t("rbac.loading")}</div>
          ) : filteredRoles.length === 0 ? (
            <div className="p-10 text-center text-muted-foreground">
              {searchQuery ? t("rbac.noMatchingRoles") : t("rbac.noRoleData")}
            </div>
          ) : (
            <div className="space-y-2">
              {paginatedRoles.map((role) => (
                <Card key={role.id} className={pageCardClass}>
                  <CardContent className="p-0">
                    ...
                    >
                      <div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-cyan-600">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {role.is_system === 1 && (
                            <Badge
                              variant="outline"
                              className="border-amber-500/25 bg-amber-500/10 text-amber-600 text-[11px]"
                            >
                              {t("rbac.systemRole")}
                            </Badge>
                          )}
                        ...
                        <p className={`text-xs ${subtleTextClass} mt-0.5 truncate`}>
                          {role.description || t("rbac.noDescription")}
                        </p>
                      </div>
                      <div className="flex items-center gap-3 shrink-0">
                        <div className="text-right">
                          <p className={`text-xs ${subtleTextClass}`}>{t("rbac.permissionCount")}</p>
                    ...
                    {expandedRoleId === role.id && (
                      <div className="border-t border-border/40 px-5 py-4">
                        ...
                        {role.permissions && role.permissions.length > 0 ? (
                          <div className="flex flex-wrap gap-1.5">
                            {role.permissions.map((p) => (
                              <Badge
                                key={p.id}
                                variant="outline"
                                className="border-cyan-500/25 bg-primary/10 text-cyan-600 text-[11px]"
                              >
                                {p.resource}:{p.action}
                              </Badge>
                            ))}
                          ...
                          <p className={`text-xs ${subtleTextClass}`}>{t("rbac.roleNoPermission")}</p>
                        )}
                      </div>
                    )}
                  </CardContent>
          ...
          {filteredRoles.length > 0 && (
            <TablePagination
              ...
              resultsLabel={t("rbac.resultsCount")}
              perPageLabel={t("rbac.paginationPerPage")}
            />
          )}
        </TabsContent>
        ...
        <TabsContent value="permissions" className="mt-4 space-y-4">
          {loading ? (
            ...
            <div className="p-10 text-center text-muted-foreground">
              {t("rbac.noPermissionData")}
            </div>
          ) : (
            <div className="space-y-3">
              {Array.from(groupedPermissions.entries()).map(([resource, perms]) => (
                <div key={resource} className="rounded-lg border border-border bg-card overflow-hidden">
                  <div className="p-0">
                    <div className="flex items-center gap-3 border-b border-border/40 px-5 py-3">
                      ...
                      <div>
                        ...
                        <p className={`text-xs ${subtleTextClass}`}>
                          {resource + " · " + perms.length + t("rbac.operations")}
                        </p>
                      </div>
                    </div>
                          ...
                          {perms.map((p) => (
                            ...
                            >
                              <td className="px-4 py-2.5 align-middle whitespace-nowrap">
                                ...
                                >
                                  {t(actionKeyMap[p.action] || p.action)}
                                </Badge>
                              </td>
                              <td className="px-4 py-2.5 align-middle whitespace-nowrap">
                                <code className="rounded bg-muted/50 px-1.5 py-0.5 text-xs text-cyan-600 font-mono">
                                  {p.resource}:{p.action}
                                </code>
                              </td>
      ...
      <ConfirmDialog
        ...
        description={t("rbac.confirmDeleteRole").replace("{{name}}", deleteTarget?.name || "")}
        level="critical"
        onConfirm={() => {
          if (deleteTarget) {
