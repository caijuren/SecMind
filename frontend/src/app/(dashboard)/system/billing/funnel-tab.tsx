    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className={String(TYPOGRAPHY.h2)}>{t("billing.conversionFunnel")}</h2>
            <p className={cn(TYPOGRAPHY.caption, "text-muted-foreground")}>
              {t("billing.funnelDesc")}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 rounded-lg bg-muted/50 p-1">
            {["7d", "30d", "90d"].map((p) => (
              ...
              >
                {p === "7d" ? t("billing.days7") : p === "30d" ? t("billing.days30") : t("billing.days90")}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <Card className={CARD.elevated}>
            <CardContent className="p-6">
              <div className="flex items-center gap-2 mb-5">
                <h3 className={cn(TYPOGRAPHY.h3, "text-foreground")}>{t("billing.funnelOverview")}</h3>
                <span className={cn(TYPOGRAPHY.micro, "text-muted-foreground")}>
                  {t("billing.totalEntered")} {formatNumber(funnelData.total_entered)}{t("billing.people")}
                </span>
              </div>
                ...
                {funnelData.stages.map((stage, i) => {
                  ...
                  return (
                    <div key={stage.stage} className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className={cn(TYPOGRAPHY.body, "font-medium text-foreground")}>
                            {stage.label}
                          </span>
                        ...
                        <div className="flex items-center gap-3">
                          <span className={cn(TYPOGRAPHY.caption, "text-muted-foreground")}>
                            {t("billing.totalConversion")} {formatPercent(stage.overall_rate)}
                          </span>
                          <span className={cn(TYPOGRAPHY.h3, "font-bold font-mono text-foreground min-w-[60px] text-right")}>
                            {formatNumber(stage.count)}
                          </span>
                        </div>
                      </div>
                      <div className="w-full h-8 rounded-lg bg-muted/50 overflow-hidden relative">
                        <div
                          className={cn(
                        ...
                        >
                          {widthPercent > 15 && (
                            <span className="pl-3 text-xs font-medium text-foreground/90">
                              {formatNumber(stage.count)}
                            </span>
                          )}
                        </div>
        ...
        <div className="space-y-4">
          <Card className={CARD.elevated}>
            <CardContent className="p-6">
              <div className="flex items-center gap-2 mb-4">
                <h3 className={cn(TYPOGRAPHY.h3, "text-foreground")}>{t("billing.keyMetrics")}</h3>
              </div>
              <div className="space-y-4">
                {(() => {
                  ...
                  return (
                    <>
                      <div className={`${softCardClass} p-4`}>
                        <p className={cn(TYPOGRAPHY.caption, "text-muted-foreground mb-1")}>{t("billing.overallConversionRate")}</p>
                        <p className={cn(TYPOGRAPHY.h1, "font-bold text-primary")}>
                          {overallRate}%
                        </p>
                        <p className={cn(TYPOGRAPHY.micro, "text-muted-foreground mt-1")}>
                          {firstStage.label} → {lastStage.label}
                        </p>
                      </div>
                      <div className={`${softCardClass} p-4`}>
                        <p className={cn(TYPOGRAPHY.caption, "text-muted-foreground mb-1")}>{t("billing.maxDropOff")}</p>
                        {maxDrop ? (
                          <>
                            <div className="flex items-center gap-2">
                            ...
                            <p className={cn(TYPOGRAPHY.h2, "font-bold font-mono text-red-600 mt-1")}>
                              {t("billing.dropOff")} {formatPercent(maxDrop.drop_rate)}
                            </p>
                            <p className={cn(TYPOGRAPHY.micro, "text-muted-foreground mt-1")}>
                              {t("billing.dropOff")} {formatNumber(maxDrop.drop_count)}{t("billing.people")}
                            </p>
                          </>
                        ) : (
                          <p className={cn(TYPOGRAPHY.body, "text-muted-foreground")}>{t("billing.noData")}</p>
                        )}
                      </div>
                      <div className={`${softCardClass} p-4`}>
                        <p className={cn(TYPOGRAPHY.caption, "text-muted-foreground mb-1")}>{t("billing.paidUsers")}</p>
                        <p className={cn(TYPOGRAPHY.h1, "font-bold text-emerald-700")}>
                          {formatNumber(lastStage.count)}
                        </p>
                        <p className={cn(TYPOGRAPHY.micro, "text-muted-foreground mt-1")}>
                          {t("billing.ofVisits")} {overallRate}%
                        </p>
                      </div>
                    </>
                  )
          ...
          <Card className={CARD.elevated}>
            <CardContent className="p-6">
              <div className="flex items-center gap-2 mb-4">
                <h3 className={cn(TYPOGRAPHY.h3, "text-foreground")}>{t("billing.dropOffDetails")}</h3>
              </div>
              <div className="space-y-3">
                {dropOffData?.drop_offs.map((item, i) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className={cn(TYPOGRAPHY.caption, "font-medium text-foreground")}>
                        {item.from_label} → {item.to_label}
                      </span>
                      {item.is_max && (
                        <Badge variant="outline" className="border-red-200 bg-red-50 text-red-600 text-[10px] px-1.5 py-0">
                          {t("billing.maxDropOffLabel")}
                        </Badge>
                      )}
                    </div>
                    <div className="flex items-center justify-between">
                      <span className={cn(TYPOGRAPHY.micro, "text-muted-foreground")}>
                        {t("billing.dropOff")} {formatNumber(item.drop_count)}{t("billing.people")}
                      </span>
      ...
      <Card className={CARD.elevated}>
        <CardContent className="p-6">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h3 className={cn(TYPOGRAPHY.h3, "text-foreground")}>{t("billing.conversionTrend")}</h3>
            <span className={cn(TYPOGRAPHY.micro, "text-muted-foreground")}>{t("billing.last90DaysWeekly")}</span>
          </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-4 flex-wrap">
                {Object.entries(TREND_COLORS).map(([stage, color]) => {
                  const label: Record<string, string> = { visit: t("billing.stageVisit"), signup: t("billing.stageSignup"), trial_start: t("billing.stageTrial"), first_analysis: t("billing.stageAnalysis"), first_response: t("billing.stageResponse"), subscribe: t("billing.stageSubscribe") }
                  return (
                    <div key={stage} className="flex items-center gap-1.5">
                      <div className="size-2.5 rounded-full" style={{ backgroundColor: color }} />
                      <span className={cn(TYPOGRAPHY.micro, "text-muted-foreground")}>{label[stage] ?? stage}</span>
                    </div>
                  )
                })}
              </div>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-muted/50">
                    <tr className="border-b border-border">
                      <th className="px-3 py-2 text-left text-muted-foreground text-xs">{t("billing.date")}</th>
                      {Object.entries(TREND_COLORS).map(([stage]) => {
                        const labelMap: Record<string, string> = { visit: t("billing.stageVisit"), signup: t("billing.stageSignup"), trial_start: t("billing.stageTrial"), first_analysis: t("billing.stageAnalysis"), first_response: t("billing.stageResponse"), subscribe: t("billing.stageSubscribe") }
                        return (
                          <th key={stage} className="px-3 py-2 text-right text-muted-foreground text-xs">{labelMap[stage] ?? stage}</th>
                        )
                      })}
                    </tr>
                  </thead>
                  <tbody>
                    {trendData.map((point, i) => (
                      <tr key={i} className="border-b border-border last:border-b-0">
                        <td className="px-3 py-2.5 text-foreground font-medium text-xs">{point.date}</td>
                        {Object.entries(TREND_COLORS).map(([stage]) => (
                          <td key={stage} className="px-3 py-2.5 text-right font-mono text-foreground text-xs">
